...
def console_program_chain_io():
    ...
    if program_chain_num == 1:
        programs = [
            ...
            ("retchannel4bin", "Do you wish to run retchannel4bin"),
            ("pulsar_det_an",  "Do you wish to run pulsar_det_an"),
    ...
    for program in program_chain:
        match program:
            case "rtl_sdr":
                ...
                exit()
            case "retchannel4bin":
                res["retchannel4bin_in_file"]          = prompt_field("Enter the file path to the input data for retchannel4bin", str)
                res["retchannel4bin_out_file"]         = prompt_field("Enter the file path you want retchannel4bin to output to", str)
                res["retchannel4bin_clock_rate"]       = prompt_field("Enter the clock rate of your data in MHz (e.g. 2)", float)
                res["retchannel4bin_OP_data_set_rate"] = prompt_field("Enter O/P data set rate in KHz (e.g. 1)", float)
                res["retchannel4bin_fft_points"]       = prompt_field("Enter the number of FFT points N (e.g. 16)", int)
            case "pulsar_det_an":
                ...
                if "retchannel4bin_out_file" in res:
                    res["pulsar_det_an_in_file"] = res["retchannel4bin_out_file"]
                else:
                ...
                res["pulsar_det_an_topo_period_ms"]   = prompt_field("Enter the topocentric period in ms output by Topobary (e.g. 714.50000)", float)
                ...
                res["pulsar_det_an_pulse_width"]      = prompt_field("Enter the ATNF pulse width (e.g. 6.5)", float)
                res["pulsar_det_an_dm"]               = prompt_field("Enter the ATNF dispersion measure DM (e.g. 26.7)", float)
                ...
                res["pulsar_det_an_rf_band_mhz"]      = prompt_field("Enter the RF band in MHz (e.g. 40)", float)
                res["pulsar_det_an_rf_center_mhz"]    = prompt_field("Enter the RF center in MHz (e.g. 35)", float)
