...
Takes rtlsdr .bin 8-bit single byte files, calculates n-point ffts, splits frequency components and averages these in time,
ready for folding. 
Outputs a 4-byte binary file of sequential frequency channelled data at the specified clock rate.

Command format:- rtlchannel4bin <rtlfile.bin> <outfile.bin> <clock rate (MHz) <downsample clock rate (kHz))><No: fft points> */
...
if(argc != 6)
    { printf("Format: rtlchannel4bin.exe <infile> <outbin> <clock rate (MHz) <downsample clock rate (kHz))><No: fft points> \n");
    exit(0);
    }
...
dsr=(DCF*1000/DSC/(float)ftpts); // ratio of clock to video number of data points averaged - or downsampling ratio
