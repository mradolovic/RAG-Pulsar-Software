...
Command Format: 
pulsar_det_an <data file> <N-point FFT> <data clock (ms))> <pulsar period (ms)> <No: sections><No. bins><pulse width><DM><ppm><spike threshold>
				<ppm range factor><RF band (MHz)><RF Centre (MHz)><roll average No.><start section><end section>

Takes N-point FFT of 4-byte .bin data file and splits data, compresses it into .txt files suitable for MathCad,Excel and/or Python analysis. 
*/
...
int num,bins,mval,PTS,Mc,mbin=0,cd,blaf[256],blas[256],mf=0,ms=0,couf,cous,rolav=8,cco=0,M,N,dmp=1,ra,c,s;
...
double comprs[32][262144],comprc[32][262144],ave[32][4096],avec[32][4096],rms[32][4096],mean[32],std[32],av[32][4096],rm[32][4096];
double compval[32][262144],compvald[32][262144],compvaldd[32][262144],spect[32][262144],puldat[256][4096],ppdot[128][128],bandat[32][4096];
double bndcum[32][4096],ftdat[1048576], ftdat2[1048576], allbands[262144],allbandsd[262144],allbandsdd[262144],spallbands[262144];
double ddispbands[256][131072],datout[8],dfold[4096],ddfold[256][4096],dfoldd[4096],dfolddd[4096],outdat[4096],fftdat[1048576];
double zero=0.000001,bstdmprof[4096],bstdmprf[4096],blasec[256],pdat[1048576],targ[1048576],foldatdd[256][4096],puldatd[256][4096];
double clck,sumt[4096],count[4096],outsumt[100][4096],pkdat[4096],sectsnr[256][8],secavsnr[256][8],foldat[256][4096],foldatd[256][4096];
...
/*check command line arguments*/
	if(argc !=17)
   		{ printf("Format: pulsar_det_an <data file> <N-point FFT> <data clock (ms))> <pulsar period (ms)> <No: sections><No. bins><pulse width><DM><ppm><spike threshold><ppm range factor><RF band (MHz)><RF Centre (MHz)><roll average No.><start section><end section>\n");
    	exit(0);
    	}
...
//File compresion to optimise good data to maximise SNR
	...
	stp=stp+1;
	nopers=(double)(Tt0*1000/period);// No. of periods in data file, 1000 factor as period input in ms
	printf("No: of Pulsar Periods = %.2f\n",nopers);
	
	per_sampls=(double)period*4.0*(double)N/(double)clck;//number of data samples in period
	sect_sampls=(double)Tt0*1000.0*4.0*(double)N/(double)M/(double)clck;//number of data samples in section
...
//Start and end section for reducing analysis section range
	start =(long long int)((long int)((double)strt*(double)ratio+0.5)*(double)per_sampls/4/N)*4*N;
	end =(long long int)((long int)((double)stp*(double)ratio+0.5)*(double)per_sampls/4/N)*4*N;
...
//Data Text Record
	...
	fprintf(fpttext,"P-dot Search Range = %.2f ppm/%d to %.2f ppm/%d \n",-(float)25.0*2.0*numlog/nno1/numper,(int)numlog,25.0*2.0*numlog/nno1/numper,(int)numlog);
	fprintf(fpttext,"Rolling Average Number = %d\n",rolav);
	fprintf(fpttext,"Start section = %ld\n",strt);
	fprintf(fpttext,"End section = %ld\n",stp-1);
	//fprintf(fpttext,"\nAttenuated Bands: ");for(d=0;d<couf;d++){fprintf(fpttext,"%d ",(int)(blaf[d]));}
	//fprintf(fpttext,"\nNoise Substitute Sections: \n");for(d=0;d<cous;d++){fprintf(fpttext,"%d ",(int)(blas[d]));}

	
//Serially fold data into M sections - http://www.y1pwe.co.uk/RAProgs/LowSNRCorrelationSearch.pdf
...
//Read input data FFT blocks	
		for(num=0;num<N;num+=1)	// N = number of FFT channels	
			...
			comprs[num][mval]=comprs[num][mval]+((double)(val*10));	//fold into bins//comprs[num][mval]=comprs[num][mval]+(double)(val-128)*100.0;
...
//Check no count entries are zero		
	...
	for(num=0;num<N;num+=1)	
		...
		for(bi=0;bi<M*bins;bi+=1)
			{
			if(comprc[num][bi] == 0)comprc[num][bi]=1.0;
			}
		}
					
//Calculate M section folded FFT channel data and section mean and rms;  M = number of sections;  
	for(num=0;num<N;num+=1)	// N = number of FFT channels
		{
		for(c=0;c<(int)(M*bins);c+=1)  //M = number of sections 
			{ 
			Mc=(int)((float)c/(float)bins);  
			compval[num][c] = (float)(comprs[num][c]/comprc[num][c]); 	// normalise raw partially folded data		
			ave[num][Mc]=ave[num][Mc]+(float)compval[num][c]/(float)bins;	// section average
			rms[num][Mc]=rms[num][Mc]+((float)(compval[num][c]))*((float)(compval[num][c]))/(float)bins;	//section squared rms						
			}
		}
...
// DC restore sections - removes long-term  receiver gain drift
	for(num=0;num<N;num+=1)	// number of FFT channels
		{
		for(c=0;c<(int)(M*bins);c+=1) //M = number of sections; bins = number of fold bins	
			{
			Mc=(int)((float)c/(float)bins);  
...
//Build section folded, DC restored raw text data file
	for(c=0;c<(int)(M*bins);c+=1)
		...
		for(num=0;num<N;num+=1)	
			{
			fprintf(fptraw,"	%f",(float)(compval[num][c])); /* write txt raw data to the output text file */		
			}
		fprintf(fptraw,"\n");			
		}
...
//Matched-filter data bandwidth to just pass pulsar pulse - FFT data and Convolve bands	
	for(num=0;num<N;num+=1)	
		...
		conv(ftdat,pulw); // outputs ftdat input blocks as convolved and filtered fftdat blocks
...
//Calculate FFT channel data and section match-filtered mean and mean square
	...
	for(num=0;num<N;num+=1)	
		...
		for(c=0;c<(M*bins);c+=1)
			...
			rm[num][Mc]=rm[num][Mc]+((float)(compval[num][c])*((float)(compval[num][c]))/bins);		
...
// Calculate new rms
	for( m=0;m<M;m+=1)
		...
		for(num=0;num<N;num+=1)	
			{
			rm[num][m]=(double)sqrt((double)rm[num][m]-(double)(av[num][m]*(double)av[num][m])); // now true rms
			}
...
//limits section band if greater than X standard deviations		
	...
	for(num=0;num<N;num+=1)
		...
		for(c=0;c<(int)(M*bins);c+=1)
			{Mc=(int)(c/bins);	
			if(compval[num][c]>(X*rm[num][Mc]))compval[num][c]=X*rm[num][Mc];
			if(-compval[num][c]>(X*rm[num][Mc]))compval[num][c]=-X*rm[num][Mc];
			}		
		}
	printf("threshold = %.1f \n",X);
...
//Build outdat.txt - compressed, match-filtered, 16-channel data text file			
	for(c=0;c<(int)(M*bins);c+=1)
		...
		for(num=0;num<N;num+=1)	
			{
			fprintf(fptout,"	%f",(float)(compval[num][c])); /* write txt data to the output text file */		
			}
			fprintf(fptout,"\n");			
		}
		...
		for(d=0;d<cous;d+=1)
			...
			for(num=0;num<N;num+=1)
				...
				for(c=0;c<M*bins;c+=1)	
					{
					Mc=(int)(c/bins);		
					if(Mc==bd){compval[num][c]= compval[num][c]/100;}
					}
		       }
			}
		}

//De-disperse recorded data based on command line DM value - de-dispered relative to median frequency
	float delta; 
	delta = 1.0*dmp*td*(float)(bins)*(float)(N-1)/(float)N/period/(float)N;  //time delay/sub-band
	printf("De-disperse Time Delay/sub-band = %.3f ms\n", delta);
	for(num=0;num<N;num+=1)	
		{
		for(c=0;c<(int)(M*bins);c+=1)
			{  
			compvaldd[num][c]=compval[num][(c+(int)(((float)((num-((float)(N)/2.0-0.5)))*delta))+M*bins)%(M*bins)];  	
			}
		}
...
//SNR per Band - Build bandS.txt
	...
	for(num=0;num<N;num+=1)	
		...
		fprintf(fptbnd,"%d	%.2f	%d\n",num,datout[0],(int)datout[1]); /* write band number SNR and max bin to bandS.txt ext file */		
		}
		printf("\n");
...
//Build De-dispersed bandat.txt band folds	
	for(num=0;num<N;num+=1)
		...
		for(d=0;d<bins;d+=1)			
			{
			fprintf(fptbndd,"	%.3f",bandat[num][d]); /* write band fold SNR data to the bandat.txt file */
			}
		fprintf(fptbndd,"\n");		
		}
...
//De-Disperse Bands - build dispersing matrix about band centre
	int emax=0,dmx=101,dmn=50;  // max and min of e range; dmp is DM polarity; dmdiv is range divider
	for(e=0;e<dmx;e+=1)	
		{
		for(num=(-N/2);num<(N/2);num+=1)	
			...
			for(c=0;c<(int)(M*bins);c+=1)
				{
				ddispbands[e][c]=ddispbands[e][c]+compval[num+N/2][(int)((c+2*M*bins+(int)((int)((float)((float)num+.5)*dmp*((float)e-(float)dmn)/dmdiv)))%(M*bins))]/N;//*20/mean[num+N/2]/N;//N;//
				}
			}
		} 
...
//Build dmSearch.txt - Dispersion Search SNR text file
	...
	for(e=0;e<dmx;e+=1)	
		...
		dmsrch= DMSch((float)((((float)(N)*((float)e-dmn)/dmdiv)))*(period/(float)bins)*(DM/td), N, DM, td, pulw);
		printf("DM =	%.2f	SNR =  %.2f	bin = %d	e = %d	dmsrch = %.2f\n",(float)((float)(N)*((float)e-dmn)/dmdiv)*(period/(float)bins)*(DM/td),datout[0],(int)datout[1],(int)((e-dmn)*dmdiv),dmsrch);//*1.0*(int)period/(float)bins*DM/td
		fprintf(fptdms,"%.2f	%.2f	%d	%d	%.2f\n",(float)((((float)(N)*((float)e-dmn)/dmdiv)))*(period/(float)bins)*(DM/td),datout[0],(int)datout[1],(int)((e-dmn)*dmdiv),dmsrch);  /* write txt data to the output text file */	
...
//Build dmprf.txt - DM peak best fold text file	
	...
	printf("\n");
		...
		{printf("Max bin = %d	Best DM SNR = %.2f	emax = %d\n",(int)(mbin),bstdmprf[(int)mbin],emax); 
		fprintf(fptmax,"%d	%.2f	%.2f	%.2f\n",(int)(mbin),(float)mmx,((float)((float)N*((float)emax-(float)dmn)*period/(float) dmdiv))/(float)bins,td); 
...
//Build dmSrchns.txt - Dispersion Search SNR - with target pulse range blanked
	for(e=0;e<dmx;e+=1)	
		...
		for(d=0;d<bins;d+=1)	
			{
			dfold[d]=ddfold[e][d];
			if(d>mbin-8*pulw && d<mbin+8*pulw)dfold[d]=0;//8 for large signalsmbin+4*pulw//30 small
			}
		psnr(bins,dfold);
		fprintf(fptdmns,"%.2f	%.2f	%d\n",(float)((float)(N)*((float)e-dmn)/dmdiv)*(period/(float)bins)*(DM/td),datout[0],(int)datout[1]);  /* write txt data to the output text file */	
		}
		printf("\n");
	for(num=(-N/2);num<(N/2);num+=1)
		{
		for(c=0;c<(int)(M*bins);c+=1)
			{
			compvald[num+N/2][c]=compval[num+N/2][(int)((c+M*bins+(int)((int)((float)(num+0.5)*dmp*((float)emax-dmn)/dmdiv)))%(M*bins))];//*20/mean[num+N/2];
			}
		}
...
//Band folds	
	int ss; 											
		for(ss=0;ss<M*bins;ss++)	
    		{
			ss=(double)ss;
			range= (long long int)(((double)ss/((double)dperiod)-(long long int)((double)ss/((double)dperiod)))*((double)bins));		
...
//Cumulative Band SNR - Build cumbands.txt	
	...
	for(num=0;num<N;num+=1)	
		...
		fprintf(fptcumf,"%d	%.2f	%d\n",num,datout[0],(int)datout[1]); /* write cumulative band data to the cumbands.txt file */
		}
		printf("\n");
...
//Section SNR. Build puldat.txt (section folds) and secsnr.txt (section SNR)
	...
	for( m=0;m<M;m+=1)
		...
		fprintf(fptsec,"%d	%.2f	%d	%.2f\n",m,datout[0],(int)datout[1],(float)thry); /* write txt data to the output text file */						
		sectsnr[m][0]=m;sectsnr[m][1]=datout[0];sectsnr[m][2]=datout[1];
		psnr(bins,dfoldd);
		for(d=0;d<bins;d+=1)
			{
			puldatd[m][d]=(float)outdat[d];
			fprintf(fptpuldd,"  %.2f",puldatd[m][d]);/* write dedispersed puldat data to the output text file */		
			}
		fprintf(fptpuldd,"\n");		
		}
...
//Cumulative Section SNR - Build cumsec.txt
	...
	for(m=0;m<M;m+=1)
		...
		fprintf(fptfold,"\n");							
		fprintf(fptcums,"%d	%.2f	%d\n",m,datout[0],(int)datout[1]); /* write cumulative band SNR txt data to the output text file */	
		psnr(bins,dfoldd);
		for(d=0;d<bins;d+=1)	
			{
		...
		for(d=0;d<bins;d+=1)	
			...
			foldatdd[m][d]=outdat[d];
     		fprintf(fptfolddd, "%.2f  ", foldatdd[m][d]);	//fprintf(fptfoldd,"\n"); // dedispersed foldat
	 	 	}
		fprintf(fptfolddd,"\n");
		}
...
//Period Search Folds
	...
	for(st=0;st<51;st++)
		...
		for(ss=0;ss<M*bins;ss++)
    		{
			ss=(double)ss;
			range= (long long int)(((double)ss/((double)pperiod)-(long long int)((double)ss/((double)pperiod)))*((double)bins));		
...
//Period search SNR. Build periodS.txt **All period p-dot search routines prefer un de-dispersed data
	double pspr;
	maxx=0;
	...
	for(st=0;st<51;st+=1)	
		...
		psnr(bins,dfold);
		if(datout[0]>maxx) maxx=datout[0];
        }
		for(st=0;st<51;st+=1)	
			{
			for(d=0;d<bins;d+=1)	
			...
			printf("ppm = %.2f	SNR =  %.2f	bin = %d\n",(float)(st-25)/nno1,datout[0],(int)datout[1]);
			fprintf(fptper," %.5f	%.5f	%d	%.3f\n",(float)(st-25)/nno1,datout[0],(int)datout[1],pspr); /* write txt data to the output text file */		
			if(st>19 && st<31)
				{
				for(d=0;d<bins;d+=1)
					{
					fprintf(fptpfold,"%f	",(float)outdat[d]); //target period folds over the selected period range			
					}
				fprintf(fptpfold,"\n"); 		
				}		
			}
...
//P-dot Search Fold
	...
	for(st=0;st<51;st++)
		...
		for(ss=0;ss<M*bins;ss++)
    		{
			ss=(double)ss;
			periodt=pperiod*(1+(double)(st-25)*1*(double)((ss*ratio/(double)(M*bins))/(double)1000000/nno1)) ;	
			range= (long long int)(((double)ss/((double)periodt)-(long long int)((double)ss/((double)periodt)))*((double)bins));		
			sumt[range]=sumt[range]+allbands[ss];
    		count[range]=count[range]+1;	
			}
		for(s=0;s<bins;s++)
...
//Pdot search SNR.  Build pdotS.txt
	...
	for(st=0;st<51;st+=1)	
		...
		psnr(bins,dfold);
		pdpr = 3+(maxx-3)*pdSch( (float)(st-25)*2*numlog*.5/(double)M/ratio/nno1, (numper), pulw, period,numlog);
		printf("pdot = %.2f	SNR =  %.2f	bin = %d\n",(float)(st-25)*2*numlog/(double)M/ratio/1/nno1,datout[0],(int)datout[1]);
		fprintf(fptpd," %.5f	%.5f	%d	%.5f\n",(float)(st-25)*2*numlog/(double)M/ratio/1/nno1,datout[0],(int)datout[1],pdpr); /* write txt data to the output text file */		
		}
		printf("\n");
...
//Build secavsnr.txt - Rolling Window/Average SNR	
	...
	for(mp=1;mp<M+1;mp+=1)
		{
		if(mp==rolav)printf("Set Section Rolling Average Window %d \n",mp);
		datout[0]=0;
		max=0;
		...
		for(m=0;m<M-mp+1;m+=1)
			...
			if(mp==rolav)
				{
				secavsnr[m][0]=m+(int)(mp/2);secavsnr[m][1]=datout[0];secavsnr[m][2]=datout[1];
				fprintf(fptavsec,"%d	%.2f	%d 	%.2f\n",m+(int)(mp/2),secavsnr[m][1],(int)secavsnr[m][2],datout[6]); /* write txt data to the output text file */	
				} 		
			cco=cco+1;
			for(d=0;d<bins;d+=1)
				{
		...
		if(mp>0)fprintf(fptrol,"%d	%.2f	%d\n",mp,max,nxx);//rolling section block average peak SNR per section
		}	
	...
	for(d=0;d<bins;d+=1)	
		{	
		fprintf(fptprof,"%.1f	%f	%f	%f\n",(float)d,(float)outdat[d],bestprof[d],pkdat[d]); /* write txt data to the output text file */		
		}	
	...
	for(m=0;m<M;m+=1)
		...
		for(c=0;c<(int)(bins);c+=1)	
			{
			fprintf(fptallb,"%f %f %f\n",(float)(allbands[c+m*bins]),(float)(allbandsd[c+m*bins]),(float)(allbandsdd[c+m*bins])); /* write txt data to the output text file */		
			fprintf(fptspallb,"%f	%f\n",((float)(c+m*bins)*1000/period/M),(float)(spallbands[c+m*bins])); /* write txt data to the output text file */
			}								
		}	
...
/*FT of folded data */
	for(v=0;v<PTS;v++)
	{
	targ[2*v] = gauss(v,(pulw*PTS/period/M),(float)(PTS/2));
	targ[2*v+1] = 0; 
	}
...
//fast fourier transform routine - see Numerical Methods in C, WH Press et al, Cambridge University Press
...
void psnr( int n, double dat[] )
	...
	n1=(int)(nx-(int)((float)(0+8*pulw*bins*1/1024)));//35 for large signals//8 small sigs
	...
	if(sqrt(rms)<0.00000001)datout[0]=0.00001;
	else datout[0]=(mx-mn)/rms;
	datout[1]=nx;
	datout[3]=mn;
	datout[4]=rms;
	datout[5]=mx;
