...
Takes rtlsdr bin files applies FFT in blocks and averages over the input data length, plot data to
log base 10. Command format:- rafft22Lg <infile> <outfile><No FFT points>*/
...
void sum_dat(void) {
    ...
    for (tt = 0; tt < pts; tt++) {
        datr[tt] =
            datr[tt] + (float)(dats[2 * tt] * dats[2 * tt] + dats[2 * tt + 1] * dats[2 * tt + 1]);
    }
}
