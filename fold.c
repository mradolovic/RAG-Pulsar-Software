...
Command format:- rapulsar2conv <infile> <outfile> <clock rate(MHz)) <No. output data points> <Pulsar period(ms)>< pulsar pulse width>
Example command line:  try313scu1.bin tr313.txt 2.4 1024 714.482438 6.5              */

//rag_obscut.bin ragobscutfold.txt 2.4 1024 714.473511 6.5   
...
//broj kosara
PTS=atoi(argv[4]);
//period foldanja isti ko period pulsara
periodf = atof(argv[5]);
//sirina pulsara
pulw = atof(argv[6]);
//mali vrem interval izmedu tocaka u kosari
...
//broj tocaka unutar kosare, dijeli ga 2 zbog iq parova
