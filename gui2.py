...
def make_info_button(parent, key, row, col):
    def on_click():
        ...
        text_box.config(state="disabled")  # read-only by default
        text_box.pack(fill="both", expand=True, padx=10, pady=(0, 4))

        # Bottom bar: Edit/Save button on the left, close on the right
        btn_frame = tk.Frame(dialog)
        btn_frame.pack(fill="x", padx=10, pady=(0, 8))

        editing = [False]  # mutable flag
        ...
        def on_close():
            # If still in edit mode when closed, save automatically
            if editing[0]:
                set_info(key, text_box.get("1.0", "end-1c"))
            dialog.destroy()
