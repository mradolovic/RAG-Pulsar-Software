...
# -----------------------------
# Info storage (persisted per session)
# -----------------------------
param_info = {}

def get_info(key):
    return param_info.get(key, "")
...
def make_info_button(parent, key, row, col):
    """Create a small ℹ button that opens an edit/view dialog for param_info[key]."""
    def on_click():
        ...
        tk.Label(dialog, text=f"Parameter: {key}", font=("TkDefaultFont", 10, "bold")).pack(anchor="w", padx=10, pady=(10, 2))
...
def run_pipeline():
    ...
    # 2. PROGRAM SELECTION — driven by checkboxes
    ...
    # 3. RTLChannel4bin inputs (only if selected)
    ...
    # 5. pulsar_det_an inputs (only if selected)
...
tk.Button(main, text="🚀 Run Full Pipeline", command=run_pipeline,
          font=("TkDefaultFont", 11, "bold"), pady=6).pack(pady=10)
